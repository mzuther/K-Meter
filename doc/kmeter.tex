...
The average level meters can either read loudness-weighted levels
according to \href{http://www.itu.int/rec/R-REC-BS.1770}{ITU-R
  BS.1770-2} (\textbf{ITU-R}) or unweighted levels (\textbf{RMS}).
Click on the corresponding radio button to make your selection.  To
reference a meter, state both K-System meter scale and averaging
method, separated by a slash, such as ``K-20/ITU''.

\emph{\underline{Note:} K-Meter implements ITU-R BS.1770-2 'K'
  frequency weighting and mean square calculation, but neither gated
  loudness, nor channel-weighted summation.}
...
\textbf{Frequency response of band-limited ITU-R BS.1770-2 detection stage
  (\SI{5}{\hertz} to \SI{96}{\kilo\hertz}, \SI{-140}{\dB} to
  \SI{5}{\dB}):}
...
\textbf{Phase response of band-limited ITU-R BS.1770-2 detection stage
  (\SI{5}{\hertz} to \SI{96}{\kilo\hertz}, \SI{-180}{\degree} to
  \SI[addsign]{+180}{\degree}):}

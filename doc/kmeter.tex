...
\emph{\underline{Note:} In ITU-R mode, the average meter is graded in
  \emph{\si{\LK}} which stands for \emph{Loudness, K weighted} and is
  by all means equivalent to \si{\dB}.}
...
The average meter uses an averaging period of \num{1024} samples.  In
\textbf{RMS} mode, this meter exhibits a flat frequency response
between \SI{20}{\hertz} and \SI{20}{\kilo\hertz} (\SI{\pm 0.01}{\dB}),
whereas the \textbf{ITU-R} mode implements 'K' frequency weighting as
specified in \href{http://www.itu.int/rec/R-REC-BS.1770}{ITU-R
  BS.1770-1}.

In any case, the average meter is band-limited using a windowed-sinc
low-pass filter with a cutoff frequency of \SI{21.0}{\kilo\hertz}.  On
level changes, it takes \SI{600}{\milli\second} for the meter to reach
\SI{99}{\percent} of the final reading.
...
\href{http://www.digido.com/media/downloads.html}{Bob Katz's download
  ...
  noise stereo \num{44.1}} and click on the \textbf{validate} button.
Please ensure that all intermediate software and hardware mixers are
...
\textbf{users of K-Meter} for sending kind words, suggestions and bug
reports.  Finally, I want to thank the \textbf{open source community}
...
using Debian 6.0 (Squeeze), but the procedure should be similar on
...
\url{http://ftp.de.debian.org/debian/} to a
...
\begin{verbatim}
  apt-get update
  apt-get install bash-completion libasound2-dev \
